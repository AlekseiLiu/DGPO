#!/bin/bash
# Experiment 2: OOD Discovery via Filtered Pretraining — NB101 (Table 2, Fig 3)
# Threshold: 0.87
#
# Usage:
#   ./scripts/run_experiment_2_nb101.sh            # full run
#   ./scripts/run_experiment_2_nb101.sh --quick     # CI smoke test
set -euo pipefail
cd "$(dirname "$0")/.."
...
if $QUICK; then
  ...
  PRETRAIN_EPOCHS=300; RLFT_EPOCHS=60; N_SAMPLES=300; LIMIT=""
fi

PRETRAIN_SEED=42
PRETRAIN_RUN="pretrain_nb101_filtered_seed${PRETRAIN_SEED}"
SEEDS=(42 123 456)
THRESHOLD=0.87
...
# ===================================================================
echo "=== Phase 1: Filtered Pretraining NB101 (threshold=${THRESHOLD}) ==="
python src/train_pretrain.py --config configs/pretrain_nb101_filtered.yaml \
...
python src/baselines.py --mode pretrained --checkpoint "$PRETRAIN_CKPT" \
  --benchmark nb101 --variant filtered --n-samples "$N_SAMPLES" --seeds "${SEEDS[@]}"

# ===================================================================
# Phase 2: RL-FT for each seed
# ===================================================================
echo "=== Phase 2: RL-FT NB101 filtered ==="
for SEED in "${SEEDS[@]}"; do
  echo "  seed=${SEED}"
  python src/train_rlft.py --config configs/rlft_nb101_filtered.yaml \
...
echo "=== Evaluation ==="
for SEED in "${SEEDS[@]}"; do
  RLFT_RUN="rlft_nb101_filtered_seed${SEED}"
  ...
  PRETRAIN_BASELINE="outputs/baselines/pretrained_nb101_filtered_seed${SEED}.json"
  PRETRAIN_ARG=""
  ...
  python src/evaluate.py \
    ...
    --config configs/rlft_nb101_filtered.yaml \
...
EXP2_STAGING="outputs/_exp2_nb101_staging"
rm -rf "$EXP2_STAGING"
...
python src/evaluate.py --aggregate \
  ...
  --expected expected_outputs/experiment_2_nb101.json \
  --output outputs/experiment_2_nb101_summary.json

echo "=== Done. Summary: outputs/experiment_2_nb101_summary.json ==="
